...
import { getModelSku, normalizeParamsForModelSku } from './lib/modelSkus'
...
import type { ServerImageTaskListItem, ServerLibraryOutputListItem } from './lib/serverImageGatewayApi'
...
const LIBRARY_ACTIVE_OUTPUT_LIMIT = 100
const LIBRARY_TRASH_RETENTION_DAYS = 7
...
function getTrashRestoreErrorMessage(error: unknown) {
  const message = error instanceof Error ? error.message : String(error)
  if (message.includes(`作品库最多保留 ${LIBRARY_ACTIVE_OUTPUT_LIMIT} 张`)) {
    return `作品库已满 ${LIBRARY_ACTIVE_OUTPUT_LIMIT} 张，请先把部分作品移入回收站后再恢复。`
  }
  return message
}
...
function resolveGatewayModelSkuId(selectedModelSkuId: string, modelSkus: ModelSku[]) {
  const normalizedSelectedModelSkuId = selectedModelSkuId.trim()
  if (normalizedSelectedModelSkuId) return normalizedSelectedModelSkuId
  return modelSkus.find((sku) => sku.enabled)?.id ?? ''
}
      ...
      Array.isArray(value.rechargeHistory)
        ? value.rechargeHistory
            ...
            .map((item) => ({
              ...
              channel: item.channel === 'recharge_code' ? item.channel : undefined,
...
function getServerOutputImageId(output: ServerPersistedImageOutput, taskId: string) {
  return `server-${output.id || `${taskId}-${output.outputIndex}`}`
}

function parseServerTaskTime(value?: string | null) {
  if (!value) return null
  const time = Date.parse(value)
  return Number.isFinite(time) ? time : null
}

function parseServerTimeOrNull(value?: string | null) {
  return parseServerTaskTime(value) ?? null
}

function mapServerTaskStatus(status: ServerImageTaskListItem['status']): TaskRecord['status'] {
  if (status === 'succeeded') return 'done'
  if (status === 'queued' || status === 'running') return 'running'
  return 'error'
}

async function persistServerTaskImageReferences(task: ServerImageTaskListItem) {
  const persistedImages = Array.isArray(task.persistedImages) ? task.persistedImages : []
  const imageIds: string[] = []
  const createdAt = parseServerTaskTime(task.createdAt) ?? Date.now()
  for (const output of persistedImages) {
    const imageUrl = typeof output.url === 'string' && output.url.trim() ? output.url.trim() : task.images?.[output.outputIndex]
    if (!imageUrl) continue
    const imageId = getServerOutputImageId(output, task.taskId)
    imageIds[output.outputIndex] = imageId
    await putImage({
      id: imageId,
      dataUrl: imageUrl,
      publicUrl: imageUrl,
...
async function mapServerTaskToTaskRecord(
  task: ServerImageTaskListItem,
  ownerUserId: string,
): Promise<TaskRecord> {
  const createdAt = parseServerTaskTime(task.createdAt) ?? Date.now()
  const finishedAt = parseServerTaskTime(task.finishedAt)
  const outputImages = await persistServerTaskImageReferences(task)
  const revisedPromptByImage = outputImages.reduce<Record<string, string>>((acc, imageId, index) => {
    const revisedPrompt = task.revisedPrompts?.[index]
    if (typeof revisedPrompt === 'string' && revisedPrompt.trim()) acc[imageId] = revisedPrompt
    return acc
  }, {})
  const status = mapServerTaskStatus(task.status)
  const errorMessage = typeof task.error?.message === 'string' && task.error.message.trim()
    ? task.error.message
    : status === 'error'
      ? '任务未完成'
      : null
  return {
    ...
    negativePrompt: typeof task.negativePrompt === 'string' ? task.negativePrompt : undefined,
    params: {
      ...DEFAULT_PARAMS,
      ...(task.params ?? {}),
    },
    ...
    requestId: task.error?.requestId || task.requestId,
    routeId: task.routeId,
    upstreamModel: task.upstreamModel,
    attempts: task.attempts,
    requestedOutputCount: task.requestedOutputCount,
    partialFailureMessage: task.partialFailureMessage,
    chargedPoints: task.billing?.chargedPoints ?? null,
    chargeLedgerId: task.billing?.ledgerId ?? null,
    gatewayFailureKind: IMAGE_GATEWAY_FAILURE_KINDS.has(task.error?.failureKind as ImageGatewayFailureKind)
      ? task.error?.failureKind as ImageGatewayFailureKind
      : undefined,
    serverImageTaskId: task.taskId,
    ...
    inputImageIds: [],
    outputImages,
    serverOutputByImageId: mapServerOutputsByImageId(outputImages, task.persistedImages),
    libraryState: 'active',
    libraryDeletedAt: null,
    libraryPurgeAfter: null,
    ...
    sourceMode: task.mode === 'agent' || task.mode === 'agent_edit' ? 'agent' : 'gallery',
  }
}

async function mapServerLibraryOutputGroupToTaskRecord(
  outputs: ServerLibraryOutputListItem[],
  ownerUserId: string,
): Promise<TaskRecord> {
  const primary = outputs[0]
  const createdAt = parseServerTaskTime(primary?.task.createdAt) ?? Date.now()
  const finishedAt = parseServerTaskTime(primary?.task.finishedAt) ?? createdAt
  const outputImages: string[] = []
  const serverOutputByImageId: NonNullable<TaskRecord['serverOutputByImageId']> = {}
  const actualParamsByImage: NonNullable<TaskRecord['actualParamsByImage']> = {}
  for (const output of outputs) {
    const imageId = `server-${output.id || `${output.taskId}-${output.outputIndex}`}`
    const outputSize =
      output.width != null && output.width > 0 && output.height != null && output.height > 0
        ? `${output.width}x${output.height}`
        : undefined
    outputImages[output.outputIndex] = imageId
    serverOutputByImageId[imageId] = {
      outputId: output.id,
      taskId: output.taskId,
      outputIndex: output.outputIndex,
    }
    if (outputSize) {
      actualParamsByImage[imageId] = { size: outputSize }
    }
    await putImage({
      id: imageId,
      dataUrl: output.url,
      publicUrl: output.url,
      createdAt,
      source: 'generated',
  ...
  const primaryOutputSize = actualParamsByImage[outputImages[0] ?? '']?.size

  return {
    id: primary.taskId,
    ownerUserId,
    prompt: primary.task.prompt || '',
    negativePrompt: primary.task.negativePrompt,
    params: { ...DEFAULT_PARAMS },
    apiProvider: 'openai',
    modelSku: primary.task.modelSku,
    requestId: primary.task.requestId ?? undefined,
    routeId: primary.task.routeId ?? undefined,
    upstreamModel: primary.task.upstreamModel ?? undefined,
    serverImageTaskId: primary.task.id,
    actualParams: primaryOutputSize ? { size: primaryOutputSize } : undefined,
    actualParamsByImage: Object.keys(actualParamsByImage).length ? actualParamsByImage : undefined,
    inputImageIds: [],
    outputImages: outputImages.filter(Boolean),
    serverOutputByImageId,
    libraryState: primary.deletedAt ? 'trashed' : 'active',
    libraryDeletedAt: parseServerTimeOrNull(primary.deletedAt),
    libraryPurgeAfter: parseServerTimeOrNull(primary.purgeAfter),
    status: primary.task.status === 'succeeded' ? 'done' : mapServerTaskStatus(primary.task.status),
    error: null,
    createdAt,
    finishedAt,
...
function mergeServerTaskIntoLocalTask(existingTask: TaskRecord, serverTask: TaskRecord): TaskRecord {
  return {
    ...existingTask,
    ...serverTask,
    id: existingTask.id,
    ownerUserId: existingTask.ownerUserId ?? serverTask.ownerUserId,
    prompt: serverTask.prompt || existingTask.prompt,
    negativePrompt: serverTask.negativePrompt ?? existingTask.negativePrompt,
    params: {
      ...existingTask.params,
      ...serverTask.params,
    },
    apiProvider: existingTask.apiProvider ?? serverTask.apiProvider,
    apiProfileId: existingTask.apiProfileId,
    apiProfileName: existingTask.apiProfileName,
    apiMode: existingTask.apiMode,
    apiModel: existingTask.apiModel,
    inputImageIds: existingTask.inputImageIds,
    maskTargetImageId: existingTask.maskTargetImageId,
    maskImageId: existingTask.maskImageId,
    sourceMode: existingTask.sourceMode ?? serverTask.sourceMode,
    agentConversationId: existingTask.agentConversationId,
    agentRoundId: existingTask.agentRoundId,
    agentMessageId: existingTask.agentMessageId,
    agentToolCallId: existingTask.agentToolCallId,
    agentBatchCallId: existingTask.agentBatchCallId,
    agentToolAction: existingTask.agentToolAction,
    isFavorite: existingTask.isFavorite,
  }
}

async function loadServerImageTasksForSession(localTasks: TaskRecord[]) {
  const state = useStore.getState()
  const sessionToken = state.authSessionToken?.trim()
  const ownerUserId = state.account.userId?.trim()
  if (!state.account.isLoggedIn || !sessionToken || !ownerUserId || !isServerImageGatewayEnabled()) return localTasks

  try {
    const { listServerImageTasks, listServerLibraryOutputs } = await import('./lib/serverImageGatewayApi')
    const [serverTasks, activeOutputs, trashedOutputs] = await Promise.all([
      listServerImageTasks(sessionToken, { limit: 100 }),
      listServerLibraryOutputs(sessionToken, { limit: 200, status: 'active' }),
      listServerLibraryOutputs(sessionToken, { limit: 200, status: 'trashed' }),
    ])
    const taskMap = new Map<string, TaskRecord>()
    const mappedTasks = await Promise.all(serverTasks.map((task) => mapServerTaskToTaskRecord(task, ownerUserId)))
    for (const task of mappedTasks) taskMap.set(task.id, task)
    const groupedOutputs = new Map<string, ServerLibraryOutputListItem[]>()
    for (const item of [...activeOutputs, ...trashedOutputs]) {
      const group = groupedOutputs.get(item.taskId) ?? []
      group.push(item)
      groupedOutputs.set(item.taskId, group)
    }
    const mappedOutputs = await Promise.all(
      Array.from(groupedOutputs.values()).map((group) =>
        mapServerLibraryOutputGroupToTaskRecord(
          [...group].sort((a, b) => a.outputIndex - b.outputIndex),
          ownerUserId,
        ),
      ),
    )
    for (const task of mappedOutputs) {
      const existing = taskMap.get(task.id)
      taskMap.set(task.id, existing ? mergeServerTaskIntoLocalTask(existing, task) : task)
    }
...
async function syncServerLibraryTasksForCurrentSession() {
  const state = useStore.getState()
  const mergedTasks = await loadServerImageTasksForSession(state.tasks)
  if (mergedTasks !== state.tasks) {
    const serverTaskIds = new Set(mergedTasks.map((task) => task.id))
    const preservedLocalTasks = state.tasks.filter((task) => {
      const hasServerOutputs = Boolean(task.serverOutputByImageId && Object.keys(task.serverOutputByImageId).length > 0)
      const isServerBacked = Boolean(task.serverImageTaskId?.trim()) || hasServerOutputs
      return !isServerBacked
    })
    state.setTasks([
      ...mergedTasks,
      ...preservedLocalTasks.filter((task) => !serverTaskIds.has(task.id)),
    ])
  }
}
...
async function refreshTaskFromServer(taskId: string) {
  const state = useStore.getState()
  const sessionToken = state.authSessionToken?.trim()
  if (!state.account.isLoggedIn || !sessionToken || !isServerImageGatewayEnabled()) return

  const localTask = state.tasks.find((task) => task.id === taskId)
  if (!localTask) return
  const remoteTaskId = localTask.serverImageTaskId?.trim() || localTask.id
  if (!remoteTaskId) return

  try {
    const { getServerImageTask } = await import('./lib/serverImageGatewayApi')
    const serverTask = await getServerImageTask(remoteTaskId, sessionToken)
    const mappedTask = await mapServerTaskToTaskRecord(
      { ...serverTask, taskId: remoteTaskId },
      localTask.ownerUserId?.trim() || state.account.userId?.trim() || '',
    )
    updateTaskInStore(taskId, mergeServerTaskIntoLocalTask(localTask, mappedTask))
  } catch {
    // Keep the current local snapshot when the backend detail cannot be refreshed.
  }
}

function getRemoteServerTaskId(task: TaskRecord) {
  const explicitTaskId = task.serverImageTaskId?.trim()
  if (explicitTaskId) return explicitTaskId
  const hasServerTrace = Boolean(
    task.chargeLedgerId
    || task.requestId
    || task.routeId
    || task.upstreamModel
    || (task.serverOutputByImageId && Object.keys(task.serverOutputByImageId).length > 0),
  )
  return hasServerTrace && task.id.trim() ? task.id.trim() : null
}

async function deleteTaskFromServerIfNeeded(task: TaskRecord) {
  const state = useStore.getState()
  const sessionToken = state.authSessionToken?.trim()
  if (!state.account.isLoggedIn || !sessionToken || !isServerImageGatewayEnabled()) return

  const remoteTaskId = getRemoteServerTaskId(task)
  if (!remoteTaskId) return

  const { deleteServerImageTask } = await import('./lib/serverImageGatewayApi')
  await deleteServerImageTask(remoteTaskId, sessionToken)
}

async function moveTaskOutputsToTrashIfNeeded(task: TaskRecord) {
  const state = useStore.getState()
  const sessionToken = state.authSessionToken?.trim()
  if (!state.account.isLoggedIn || !sessionToken || !isServerImageGatewayEnabled()) return false
  const outputs = task.serverOutputByImageId ? Object.values(task.serverOutputByImageId) : []
  if (!outputs.length) return false
  const { deleteServerLibraryOutput } = await import('./lib/serverImageGatewayApi')
  await Promise.all(outputs.map((output) => deleteServerLibraryOutput(output.outputId, sessionToken)))
  return true
}
...
async function recordAgentCompletedImageTask(input: {
  ...
  try {
    ...
    if (!result.alreadyRecorded && result.chargedPoints > 0) {
      void useStore.getState().refreshBackendAccount()
    }
...
export async function stopRunningTask(task: TaskRecord, options: { silent?: boolean } = {}) {
...
export function migratePersistedState(persistedState: unknown): unknown {
  ...
  const { accountProfiles: _legacyAccountProfiles, ...restPersistedState } = persistedState
  const authSessionToken = typeof persistedState.authSessionToken === 'string' && persistedState.authSessionToken.trim()
    ? persistedState.authSessionToken.trim()
    : null
  const normalizedAccount = normalizeAccountState(persistedState.account, DEFAULT_ACCOUNT_STATE)
  const account = authSessionToken ? normalizedAccount : { ...DEFAULT_ACCOUNT_STATE }
  const billing = authSessionToken
    ? normalizeBillingState(persistedState.billing, DEFAULT_BILLING_STATE)
  ...
  return {
    ...
    constraintMemoryTerms: Array.isArray(persistedState.constraintMemoryTerms)
      ...
      : [],
    pinnedConstraintTerms: Array.isArray(persistedState.pinnedConstraintTerms)
      ? persistedState.pinnedConstraintTerms.filter((item): item is string => typeof item === 'string')
      : [],
    recentSizePresets: Array.isArray(persistedState.recentSizePresets)
      ? persistedState.recentSizePresets.filter((item): item is string => typeof item === 'string')
      : [],
    pinnedSizePresets: Array.isArray(persistedState.pinnedSizePresets)
      ? persistedState.pinnedSizePresets.filter((item): item is string => typeof item === 'string')
      : [],
  }
}
...
export function getPersistedState(state: AppState) {
  ...
  return {
    ...
    pinnedConstraintTerms: state.pinnedConstraintTerms,
    recentSizePresets: state.recentSizePresets,
    pinnedSizePresets: state.pinnedSizePresets,
    ...(settings.persistInputOnRestart && (state.appMode === 'gallery' || galleryInputDraft)
      ? {
          prompt: galleryInputDraft?.prompt ?? '',
          negativePrompt: galleryInputDraft?.negativePrompt ?? '',
...
function mergePersistedState(persistedState: unknown, currentState: AppState): AppState {
  ...
  const authSessionToken = typeof persisted.authSessionToken === 'string' && persisted.authSessionToken.trim()
    ...
    : null
  const account = authSessionToken
    ? normalizeAccountState(persisted.account, currentState.account)
    : { ...DEFAULT_ACCOUNT_STATE }
  const billing = getFreshBillingState()
  ...
  const constraintMemoryTerms = Array.isArray(persisted.constraintMemoryTerms)
    ? persisted.constraintMemoryTerms
        ...
        .filter(Boolean)
        .slice(0, 8)
    : currentState.constraintMemoryTerms
  const pinnedConstraintTerms = Array.isArray(persisted.pinnedConstraintTerms)
    ? persisted.pinnedConstraintTerms
        .filter((item): item is string => typeof item === 'string')
        .map((item) => item.trim())
        .filter(Boolean)
        .slice(0, 8)
    : currentState.pinnedConstraintTerms
  const recentSizePresets = Array.isArray(persisted.recentSizePresets)
    ? persisted.recentSizePresets
        .filter((item): item is string => typeof item === 'string')
        .map((item) => item.trim())
        .filter(Boolean)
        .slice(0, 3)
    : currentState.recentSizePresets
  const pinnedSizePresets = Array.isArray(persisted.pinnedSizePresets)
    ? persisted.pinnedSizePresets
        .filter((item): item is string => typeof item === 'string')
        .map((item) => item.trim())
        .filter(Boolean)
        .slice(0, 3)
  ...
  const galleryInputDraft = settings.persistInputOnRestart
    ? normalizeAgentInputDraft(persisted.galleryInputDraft ?? {
        ...
        negativePrompt: persisted.negativePrompt,
  ...
  return {
    ...
    constraintMemoryTerms,
    pinnedConstraintTerms,
    recentSizePresets,
    pinnedSizePresets,
...
interface AppState {
  ...
  accountLedgerError: string | null
  accountLedgerLoading: boolean
  ...
  reusedNegativePromptSource: { prompt: string; negativePrompt: string } | null
  constraintMemoryTerms: string[]
  pushConstraintMemoryTerms: (terms: string[]) => void
  clearConstraintMemoryTerms: () => void
  pinnedConstraintTerms: string[]
  togglePinnedConstraintTerm: (term: string) => void
  recentSizePresets: string[]
  pushRecentSizePreset: (size: string) => void
  pinnedSizePresets: string[]
  togglePinnedSizePreset: (size: string) => void
  inputImages: InputImage[]
...
function isEmptyAgentInputDraft(draft: AgentInputDraft) {
  return draft.prompt.length === 0 && (draft.negativePrompt ?? '').length === 0 && draft.inputImages.length === 0 && !draft.maskDraft && !draft.maskEditorImageId
}
  ...
  persist(
    (set, get) => ({
      ...
      setAccountState: (patch) => set((state) => {
        ...
        const shouldResetBillingForAccountChange = !isSameAccount && (
          Boolean(state.authSessionToken?.trim())
          || Boolean(state.account.email?.trim())
          || Boolean(account.email?.trim())
        )
        const billing = shouldResetBillingForAccountChange ? getFreshBillingState() : state.billing
        return {
          account,
          billing,
          ...(!isSameAccount ? {
      ...
      refreshBackendAccount: async () => {
        const state = get()
        const token = state.authSessionToken?.trim()
        if (!token) return

        try {
          const { accountFromAuthSnapshot, getCurrentAuthAccount } = await import('./lib/authApi')
          const payload = await getCurrentAuthAccount(token)
          get().setAccountState(accountFromAuthSnapshot(payload))
        } catch (error) {
          get().logout()
          get().showToast(error instanceof Error ? error.message : '登录状态已失效，请重新登录', 'info')
        }
      },
      refreshAccountLedger: async () => {
        const state = get()
        const token = state.authSessionToken?.trim()
        if (!state.account.isLoggedIn || !token) {
          set({ accountLedger: null, accountLedgerError: null, accountLedgerLoading: false })
          return
        }

        set({ accountLedger: null, accountLedgerError: null, accountLedgerLoading: true })
        try {
          const { getAccountLedger } = await import('./lib/authApi')
          const ledger = await getAccountLedger(token, 100)
          set({ accountLedger: ledger, accountLedgerError: null, accountLedgerLoading: false })
        } catch (error) {
          set({
            accountLedger: null,
            accountLedgerError: error instanceof Error ? error.message : '余额流水加载失败',
            accountLedgerLoading: false,
          })
        }
      },
      refreshReferralInfo: async () => {
        const state = get()
        const token = state.authSessionToken?.trim()
        if (!state.account.isLoggedIn || state.account.inviteCode?.trim() || !token) return

        try {
          const { getMyReferralInfo } = await import('./lib/authApi')
          const payload = await getMyReferralInfo(token)
          get().setAccountState({ inviteCode: payload.referral.inviteCode })
        } catch {
          get().showToast('邀请信息加载失败，请稍后刷新账号状态', 'info')
        }
      },
      ...
      redeemRechargeCode: async (code) => {
        ...
        if (!token) {
          const now = Date.now()
          const rechargeErrorMessage = '请登录真实账号后再兑换余额码'
          const nextRecord: BillingState['rechargeHistory'][number] = {
            id: genId(),
            amount: normalizeRechargePackageAmount(state.billing.pendingRechargeAmount),
            status: 'failed',
          ...
          state.showToast(rechargeErrorMessage, 'error')
          return
        }
        ...
        const { redeemRechargeCodeWithApi } = await import('./lib/rechargeCodeApi')

        try {
          const result = await redeemRechargeCodeWithApi(normalizedCode, token)
          ...
          await get().refreshBackendAccount()
          await get().refreshAccountLedger()
      ...
      setPrompt: (prompt) => set((s) => {
        const reusedNegativePromptSource = s.reusedNegativePromptSource
        if (
          reusedNegativePromptSource
          && s.negativePrompt === reusedNegativePromptSource.negativePrompt
          && prompt.trim() !== reusedNegativePromptSource.prompt.trim()
        ) {
          return {
            ...syncActiveInputDraft(s, { prompt, negativePrompt: '' }),
            reusedNegativePromptSource: null,
          }
        }
        return syncActiveInputDraft(s, { prompt })
      }),
      negativePrompt: '',
      setNegativePrompt: (negativePrompt) => set((s) => ({
        ...syncActiveInputDraft(s, { negativePrompt }),
        reusedNegativePromptSource: null,
      })),
      reusedNegativePromptSource: null,
      constraintMemoryTerms: [],
      pushConstraintMemoryTerms: (terms) =>
        set((s) => ({
          constraintMemoryTerms: mergeNegativePromptMemoryTerms(s.constraintMemoryTerms, terms),
        })),
      clearConstraintMemoryTerms: () => set({ constraintMemoryTerms: [] }),
      pinnedConstraintTerms: [],
      togglePinnedConstraintTerm: (term) =>
        set((s) => ({
          pinnedConstraintTerms: togglePinnedNegativePromptTerm(s.pinnedConstraintTerms, term),
        })),
      recentSizePresets: [],
      pushRecentSizePreset: (size) =>
        set((s) => ({
          recentSizePresets: mergeRecentSizePresets(s.recentSizePresets, size),
        })),
      pinnedSizePresets: [],
      togglePinnedSizePreset: (size) =>
        set((s) => ({
          pinnedSizePresets: togglePinnedSizePresetItem(s.pinnedSizePresets, size),
        })),
      inputImages: [],
      addInputImage: (img) =>
        set((s) => {
      ...
      loadModelSkus: async () => {
        try {
          ...
          const { modelSkus, defaultModelSkuId } = await fetchPublicModelSkus()
          set((state) => {
            const selectedStillAvailable = modelSkus.some((sku) => sku.enabled && sku.id === state.selectedModelSkuId)
            const defaultStillAvailable = defaultModelSkuId
              ? modelSkus.some((sku) => sku.enabled && sku.id === defaultModelSkuId)
              : false
            const selectedModelSkuId = selectedStillAvailable
              ? state.selectedModelSkuId
              : defaultStillAvailable
                ? defaultModelSkuId as string
                : modelSkus.find((sku) => sku.enabled)?.id ?? ''
            return {
              modelSkus,
      ...
      setSelectedModelSkuId: (selectedModelSkuId) => set((state) => {
        const nextModelSku = getModelSku(selectedModelSkuId, state.modelSkus)
        const clearsMaskDraft = Boolean(state.maskDraft) && nextModelSku?.supportsMask === false
        const blocksReferenceEditing = state.inputImages.length > 0 && nextModelSku?.supportsEdit === false
        const nextState = syncActiveInputDraft(state, {
          ...(clearsMaskDraft ? { maskDraft: null, maskEditorImageId: null } : {}),
        })
        if (clearsMaskDraft) {
          state.showToast('当前模型不支持遮罩编辑，已清除现有遮罩草稿。', 'info')
        } else if (blocksReferenceEditing) {
          state.showToast('当前模型不支持参考图编辑，已保留参考图，但暂不可提交图生图。', 'info')
        }
        return {
          ...nextState,
          selectedModelSkuId,
      ...
      setDetailTaskId: (detailTaskId) => {
        ...
        set({ detailTaskId })
        if (detailTaskId) void get().refreshTaskFromServer(detailTaskId)
      },
      lightboxImageId: null,
      lightboxImageList: [],
...
function getGatewayErrorPublicInfo(err: unknown): Pick<Partial<TaskRecord>, 'gatewayFailureKind' | 'routeId' | 'upstreamModel' | 'attempts'> & { requestId?: string } {
  if (!(err instanceof Error)) return {}

  const requestId = 'requestId' in err && typeof (err as { requestId?: unknown }).requestId === 'string'
    ? (err as { requestId: string }).requestId
    : undefined
  const routeIdFromError = 'routeId' in err && typeof (err as { routeId?: unknown }).routeId === 'string'
    ? (err as { routeId: string }).routeId
    : undefined
  const upstreamModelFromError = 'upstreamModel' in err && typeof (err as { upstreamModel?: unknown }).upstreamModel === 'string'
    ? (err as { upstreamModel: string }).upstreamModel
    : undefined
  const attempts = 'attempts' in err && Array.isArray((err as { attempts?: unknown }).attempts)
    ? (err as { attempts: TaskRecord['attempts'] }).attempts
    : undefined
  const latestAttempt = attempts?.length ? attempts[attempts.length - 1] : undefined
  const routeId = routeIdFromError ?? latestAttempt?.routeId
  const upstreamModel = upstreamModelFromError ?? latestAttempt?.upstreamModel
...
function readSvgDataUrlSizeParam(dataUrl: string): Partial<TaskParams> | undefined {
  if (!dataUrl.startsWith('data:image/svg+xml')) return undefined
...
async function hydrateActualImageParamsList(
  images: string[],
  ...
  return images.map((_, index) => {
    const preferredParams = preferred?.[index]
    const measuredParams = measured[index]
    const merged = {
      ...(hasActualParams(preferredParams) ? preferredParams : {}),
      ...(hasActualParams(measuredParams) ? measuredParams : {}),
    }
    return hasActualParams(merged) ? merged : undefined
  })
}

async function readImageSizeParam(dataUrl: string): Promise<Partial<TaskParams> | undefined> {
  const svgSize = readSvgDataUrlSizeParam(dataUrl)
...
export async function initStore() {
  ...
  const localTasks = markedTasks.map(getPersistableTask)
  ...
  const tasks = await loadServerImageTasksForSession(localTasks)
...
export async function submitTask(options: { allowFullMask?: boolean; useCurrentApiProfileWhenReusedMissing?: boolean } = {}) {
  ...
  const gatewayModelSkuId = resolveGatewayModelSkuId(selectedModelSkuId, modelSkus)
  const activeModelSku = getModelSku(gatewayModelSkuId, modelSkus)
  const hasGatewayPath = isServerImageGatewayEnabled()
  ...
  if (hasGatewayPath && inputImages.length > 0 && activeModelSku?.supportsEdit === false) {
    showToast('当前模型「' + activeModelSku.label + '」不支持参考图编辑，请切换支持编辑的模型。', 'error')
    return
  }

  if (hasGatewayPath && maskDraft && activeModelSku?.supportsMask === false) {
    showToast('当前模型「' + activeModelSku.label + '」不支持遮罩编辑，请切换支持遮罩的模型。', 'error')
    return
  }
  ...
  const normalizedParams = hasGatewayPath
    ? normalizeParamsForModelSku(params, gatewayModelSkuId, modelSkus.length ? modelSkus : undefined)
  ...
  const taskId = genId()
  const sanitizedNegativePrompt = sanitizeNegativePrompt(negativePrompt, prompt.trim())
  const task: TaskRecord = {
    ...
    modelSku: gatewayModelSkuId || undefined,
  ...
  if (settings.clearInputAfterSubmit) {
    ...
    useStore.getState().setNegativePrompt('')
...
export async function submitAgentMessage() {
  ...
  const round: AgentRound = {
    ...
    userMessageId,
    prompt: trimmedNegativePrompt ? `${trimmedPrompt}\n\nNegative prompt: ${trimmedNegativePrompt}` : trimmedPrompt,
    inputImageIds,
    maskTargetImageId,
    maskImageId,
    outputTaskIds: [],
  ...
  const userMessage: AgentMessage = {
    id: userMessageId,
    role: 'user',
    content: trimmedNegativePrompt ? `${trimmedPrompt}\n\nNegative prompt: ${trimmedNegativePrompt}` : trimmedPrompt,
    roundId,
    inputImageIds,
    maskTargetImageId,
    maskImageId,
  ...
  try {
    ...
    const completeAgentImageTask = async (image: AgentApiResultImage, rawResponsePayload?: string) => {
      ...
      updateTaskInStore(taskId, {
        ...
        chargedPoints: serverBilling?.chargedPoints ?? null,
        chargeLedgerId: serverBilling?.ledgerId ?? null,
      ...
      if (serverBilling?.persistedImages?.length) {
        void syncServerLibraryTasksForCurrentSession()
      }
      return taskId
    }
...
async function executeTask(taskId: string) {
  ...
  try {
    ...
    const sanitizedNegativePrompt = sanitizeNegativePrompt(task.negativePrompt, task.prompt)
    ...
    if (preferServerGateway) {
      const gatewayRequest = {
        ...(task.modelSku ? { modelSku: task.modelSku } : {}),
        prompt: promptForApi,
        negativePrompt: sanitizedNegativePrompt,
        params: task.params,
    ...
    if (gatewayResult) {
      updateTaskInStore(taskId, {
        ...
        apiProvider: 'openai',
        apiMode: 'images',
        requestId: gatewayResult.taskId || gatewayResult.routeHealth?.requestId || gatewayResult.routeSelection?.requestId,
        routeId: gatewayResult.routeId,
        upstreamModel: gatewayResult.upstreamModel,
    ...
    const actualParamsList = taskProvider === 'fal'
      ...
      : await hydrateActualImageParamsList(result.images, result.actualParamsList)
    const actualParams = (() => {
    ...
    updateTaskInStore(taskId, {
      ...
      requestId: gatewayPartialResult.taskId || gatewayPartialResult.routeHealth?.requestId || gatewayPartialResult.routeSelection?.requestId,
      routeId: gatewayPartialResult.routeId,
      upstreamModel: gatewayPartialResult.upstreamModel,
      attempts: gatewayPartialResult.attempts,
      requestedOutputCount: gatewayPartialResult.requestedOutputCount,
      partialFailureMessage: gatewayPartialResult.partialFailureMessage,
      deliveryPlan: gatewayPartialResult.deliveryPlan,
      chargedPoints: serverBilling?.chargedPoints ?? null,
      chargeLedgerId: serverBilling?.ledgerId ?? null,
      actualParams,
      actualParamsByImage,
    ...
    if ((result as ImageGatewayResult).persistedImages?.length) {
      void syncServerLibraryTasksForCurrentSession()
    }
    ...
    if (latestTask.apiProvider === 'fal' && latestFalRequestInfo && isFalConnectionRecoverableError(err)) {
      updateTaskInStore(taskId, {
        ...
        requestId: latestFalRequestInfo.requestId,
      ...
      updateTaskInStore(taskId, {
        ...
        routeId: gatewayInfo.routeId,
        upstreamModel: gatewayInfo.upstreamModel,
        attempts: gatewayInfo.attempts,
...
function applyLocalUsageCharge(
  ...
  if (state.authSessionToken?.trim() && hasServerBilling) {
    if (serverBilling.chargedPoints > 0) {
      void useStore.getState().refreshAccountLedger()
    }
    return
  }
...
export async function reuseConfig(task: TaskRecord) {
  ...
  const sanitizedNegativePrompt = sanitizeNegativePrompt(task.negativePrompt, task.prompt)
  setNegativePrompt(sanitizedNegativePrompt ?? '')
  useStore.setState({
    reusedNegativePromptSource: sanitizedNegativePrompt
      ? { prompt: task.prompt, negativePrompt: sanitizedNegativePrompt }
      : null,
  })
...
function parseNegativePromptTerms(value: string) {
  return value
    .split(/[\n,，、;；]+/)
    .map((item) => item.trim())
    .filter(Boolean)
...
export function mergeNegativePromptTerms(existingValue: string, terms: string[]) {
  ...
  for (const term of [...parseNegativePromptTerms(existingValue), ...terms.map((item) => item.trim()).filter(Boolean)]) {
...
export function mergeNegativePromptValue(existingValue: string, incomingValue: string, prompt = '') {
  const merged = mergeNegativePromptTerms(existingValue, parseNegativePromptTerms(incomingValue))
  return sanitizeNegativePrompt(merged, prompt) ?? ''
}

function mergeNegativePromptMemoryTerms(existingTerms: string[], terms: string[]) {
  const incoming = terms.map((item) => item.trim()).filter(Boolean)
  if (incoming.length === 0) return existingTerms

  const next: string[] = []
  const seen = new Set<string>()

  for (const term of [...incoming.reverse(), ...existingTerms]) {
    const key = term.toLowerCase()
    if (seen.has(key)) continue
    seen.add(key)
    next.push(term)
...
function togglePinnedNegativePromptTerm(existingTerms: string[], term: string) {
  const normalized = term.trim()
  if (!normalized) return existingTerms
  const key = normalized.toLowerCase()
  const exists = existingTerms.some((item) => item.toLowerCase() === key)
  if (exists) return existingTerms.filter((item) => item.toLowerCase() !== key)
  return [normalized, ...existingTerms].slice(0, 8)
}

function mergeRecentSizePresets(existingSizes: string[], size: string) {
  const normalized = size.trim()
  if (!normalized) return existingSizes

  const next = [normalized, ...existingSizes.filter((item) => item.toLowerCase() !== normalized.toLowerCase())]
  return next.slice(0, 3)
}

function togglePinnedSizePresetItem(existingSizes: string[], size: string) {
  const normalized = size.trim()
  if (!normalized) return existingSizes
  const key = normalized.toLowerCase()
  const exists = existingSizes.some((item) => item.toLowerCase() === key)
  if (exists) return existingSizes.filter((item) => item.toLowerCase() !== key)
  return [normalized, ...existingSizes].slice(0, 3)
}

export function appendNegativePromptTerms(terms: string[], sourceLabel = '已回填到负面提示词') {
  const { negativePrompt, setNegativePrompt, pushConstraintMemoryTerms, showToast } = useStore.getState()
  const nextValue = mergeNegativePromptTerms(negativePrompt, terms)
...
export async function removeMultipleTasks(taskIds: string[]) {
  ...
  for (const task of tasks.filter((item) => taskIds.includes(item.id))) {
    if (task.status === 'running') {
      await stopRunningTask(task)
      stoppedTaskIds.add(task.id)
      continue
    }
    try {
      await deleteTaskFromServerIfNeeded(task)
      confirmedTaskIds.push(task.id)
    } catch (error) {
      failedTaskErrors.push(error instanceof Error ? error.message : '删除服务端任务失败')
    }
  }

  if (!confirmedTaskIds.length) {
    if (failedTaskErrors.length > 0) {
      showToast(failedTaskErrors[0], 'error')
      return
    }
    if (stoppedTaskIds.size > 0) {
      showToast(`已停止等待 ${stoppedTaskIds.size} 条任务，保留记录供后续查看`, 'info')
    }
    return
  }

  const latestTasks = useStore.getState().tasks
  const toDelete = new Set(confirmedTaskIds)
  const deletedTasks = latestTasks.filter(t => toDelete.has(t.id))
  const remaining = await scrubAgentOutputPayloadsForDeletedTasks(deletedTasks, latestTasks.filter(t => !toDelete.has(t.id)))
  ...
  const summaryParts = [`已删除 ${confirmedTaskIds.length} 条记录`]
  if (stoppedTaskIds.size > 0) summaryParts.push(`停止等待 ${stoppedTaskIds.size} 条`)
  if (failedTaskErrors.length > 0) summaryParts.push(`失败 ${failedTaskErrors.length} 条`)
  showToast(summaryParts.join('，'), failedTaskErrors.length > 0 ? 'info' : 'success')
}
...
export async function removeTask(task: TaskRecord) {
  ...
  if (hasServerOutputs) {
    try {
      const moved = await moveTaskOutputsToTrashIfNeeded(task)
      if (moved) {
        updateTaskInStore(task.id, {
          libraryState: 'trashed',
          libraryDeletedAt: Date.now(),
          libraryPurgeAfter: Date.now() + LIBRARY_TRASH_RETENTION_DAYS * 24 * 60 * 60 * 1000,
        })
        showToast(`作品已移入回收站，${LIBRARY_TRASH_RETENTION_DAYS} 天后清理`, 'success')
        return
      }
    } catch (error) {
      showToast(error instanceof Error ? error.message : '移入回收站失败', 'error')
      return
    }
  }
  try {
    ...
    showToast(error instanceof Error ? error.message : '删除服务端任务失败', 'error')
    return
  }
...
export async function restoreTaskFromTrash(task: TaskRecord) {
  const state = useStore.getState()
  const sessionToken = state.authSessionToken?.trim()
  const outputs = task.serverOutputByImageId ? Object.values(task.serverOutputByImageId) : []
  if (!state.account.isLoggedIn || !sessionToken || !outputs.length || !isServerImageGatewayEnabled()) {
    state.showToast('当前记录不支持从回收站恢复', 'error')
    return
  }
  try {
    const { restoreServerLibraryOutput } = await import('./lib/serverImageGatewayApi')
    await Promise.all(outputs.map((output) => restoreServerLibraryOutput(output.outputId, sessionToken)))
    updateTaskInStore(task.id, {
      libraryState: 'active',
      libraryDeletedAt: null,
      libraryPurgeAfter: null,
    })
    state.showToast('作品已恢复到作品库', 'success')
  } catch (error) {
    state.showToast(getTrashRestoreErrorMessage(error) || '恢复作品失败', 'error')
  }
}
...
export async function clearData(options: ClearOptions = { clearConfig: true, clearTasks: true }) {
  ...
  if (options.clearTasks) {
    const state = useStore.getState()
    const sessionToken = state.authSessionToken?.trim()
    if (state.account.isLoggedIn && sessionToken && isServerImageGatewayEnabled()) {
      const runningTasks = state.tasks.filter((task) => task.status === 'running')
      for (const task of runningTasks) {
        await stopRunningTask(task, { silent: true })
      }

      try {
        const { deleteAllCompletedServerImageTasks } = await import('./lib/serverImageGatewayApi')
        await deleteAllCompletedServerImageTasks(sessionToken)
      } catch (error) {
        showToast(error instanceof Error ? error.message : '清空服务端任务失败', 'error')
        return
      }
    }
