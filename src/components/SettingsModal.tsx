...
type ApiDiagnosticKind = 'connection' | 'image'
...
function truncateDiagnosticDetail(value: string, maxLength = 500) {
  const trimmed = value.trim()
  if (!trimmed) return ''
  return trimmed.length > maxLength ? `${trimmed.slice(0, maxLength)}...` : trimmed
}

function getImageDiagnosticPath(settings: AppSettings, profile: ApiProfile) {
  if (profile.provider === 'openai') {
    return profile.apiMode === 'responses' ? 'responses' : 'images/generations'
  }

  const customProvider = getCustomProviderDefinition(settings, profile.provider)
  return customProvider?.submit.path || 'images/generations'
}

function getConnectionDiagnosticNote(status?: number) {
  if (status === 401 || status === 403) return '服务器可达，但 API Key 无效，或当前账号没有访问权限。'
  if (status === 404) return '服务器可达，但 `/v1/models` 路径不存在。部分中转站不提供该路由，可继续测试生图接口。'
  if (status === 429) return '服务器可达，但被限流或额度不足。'
  if (status && status >= 500) return '服务器可达，但网关或上游服务当前异常。'
  return '连接测试失败。'
}

function getImageDiagnosticNote(status?: number) {
  if (status === 400) return '图像接口已收到请求，但参数、模型名或请求体格式不符合该服务要求。'
  if (status === 401 || status === 403) return '图像接口可达，但 API Key 无效，或当前账号没有图像权限。'
  if (status === 404) return '图像接口路径不存在，当前服务可能不支持 `/v1/images/generations` 或兼容路由不同。'
  if (status === 429) return '图像接口可达，但额度不足或被限流。'
  if (status && status >= 500) return '图像接口可达，但网关或上游图像服务当前异常。'
  return '图像接口测试失败。'
}

async function runConnectionDiagnostic(settings: AppSettings): Promise<ApiDiagnosticResult> {
  const profile = getActiveApiProfile(settings)
  const proxyConfig = readClientDevProxyConfig()
  const useApiProxy = shouldUseApiProxy(profile.apiProxy, proxyConfig)
  const requestUrl = buildApiUrl(profile.baseUrl, 'models', proxyConfig, useApiProxy)
  ...
  try {
    ...
    if (!response.ok) {
      return {
        ...
        detail: truncateDiagnosticDetail(await getApiErrorMessage(response)),
        elapsedMs,
        note: getConnectionDiagnosticNote(response.status),
      }
    }
    ...
    return {
      ...
      detail: '连接与鉴权通过。',
      elapsedMs,
      note: useApiProxy ? '当前请求通过 API 代理转发。' : '当前请求直连上游地址。',
    }
  } catch (error) {
    return {
      kind: 'connection',
      ...
      method: 'GET',
      requestUrl,
      detail: truncateDiagnosticDetail(error instanceof Error ? error.message : String(error)),
      elapsedMs: Date.now() - startedAt,
      note: '请求未到达接口，通常是地址错误、网络失败、TLS 证书异常或本地环境拦截。',
    }
  }
}

async function runImageDiagnostic(settings: AppSettings): Promise<ApiDiagnosticResult> {
  const profile = getActiveApiProfile(settings)
  const proxyConfig = readClientDevProxyConfig()
  const useApiProxy = shouldUseApiProxy(profile.apiProxy, proxyConfig)
  const path = getImageDiagnosticPath(settings, profile)
  const requestUrl = buildApiUrl(profile.baseUrl, path, proxyConfig, useApiProxy)
  const startedAt = Date.now()

  const requestBody = profile.apiMode === 'responses'
    ? {
        model: profile.model,
        input: 'Generate a minimal diagnostic image: a small gray circle on a plain white background.',
        tools: [{ type: 'image_generation' }],
      }
    : {
        model: profile.model,
        prompt: 'Generate a minimal diagnostic image: a small gray circle on a plain white background.',
        size: '1024x1024',
        quality: 'low',
        n: 1,
      }
  ...
  try {
    ...
    if (!response.ok) {
      return {
        ...
        detail: truncateDiagnosticDetail(await getApiErrorMessage(response)),
        elapsedMs,
        note: getImageDiagnosticNote(response.status),
      }
    }
    ...
    return {
      ...
      status: response.status,
      statusText: response.statusText,
      detail: '图像生成接口已返回成功响应。',
      elapsedMs,
      note: '该测试已真实调用生图接口，可能会消耗一次额度。',
    }
  } catch (error) {
    return {
      kind: 'image',
      ...
      detail: truncateDiagnosticDetail(error instanceof Error ? error.message : String(error)),
      elapsedMs: Date.now() - startedAt,
      note: '请求未到达图像接口，通常是地址错误、网络失败、TLS 证书异常或本地环境拦截。',
    }
  }
}
...
function normalizePersonalDraft(nextDraft: AppSettings): AppSettings {
  if (!PERSONAL_USE_MODE) return nextDraft

  const profiles = nextDraft.profiles.map((profile, index) => ({
    ...profile,
    provider: 'openai' as const,
    id: profile.id || (index === 0 ? DEFAULT_OPENAI_PROFILE_ID : newId('openai')),
  }))

  const activeProfileId = profiles.some((profile) => profile.id === nextDraft.activeProfileId)
    ? nextDraft.activeProfileId
    : (profiles[0]?.id ?? DEFAULT_OPENAI_PROFILE_ID)
...
export default function SettingsModal() {
  ...
  const [runningDiagnostic, setRunningDiagnostic] = useState<ApiDiagnosticKind | null>(null)
  ...
  const visibleCustomProviders = PERSONAL_USE_MODE ? [] : draft.customProviders
  const defaultProviderOrder = ['openai', ...(PERSONAL_USE_MODE ? [] : ['fal']), ...visibleCustomProviders.map(p => p.id)]
  const providerOrder = draft.providerOrder || defaultProviderOrder

  const unorderedProviderOptions = [
    { label: 'OpenAI 兼容接口', value: 'openai', draggable: true },
    ...(PERSONAL_USE_MODE ? [] : [{ label: 'fal.ai', value: 'fal', draggable: true }]),
  ...
  const providerOptions = [
    ...(!PERSONAL_USE_MODE ? [{ label: '创建自定义服务商', value: ADD_CUSTOM_PROVIDER_VALUE, variant: 'action' as const }] : []),
  ...
  const runApiDiagnostic = useCallback(async (kind: ApiDiagnosticKind) => {
    const effectiveSettings = normalizeSettings({
      ...draft,
      profiles: draft.profiles.map((profile) => profile.id === activeProfile.id ? {
        ...activeProfile,
        timeout: Number(timeoutInput) || activeProfile.timeout,
      } : profile),
    ...
    try {
      const result = kind === 'connection'
        ? await runConnectionDiagnostic(effectiveSettings)
        : await runImageDiagnostic(effectiveSettings)

      const lines = [
        `结果：${result.ok ? '成功' : '失败'}`,
        `请求：${result.method} ${result.requestUrl}`,
        `耗时：${(result.elapsedMs / 1000).toFixed(2)} 秒`,
        ...(typeof result.status === 'number' ? [`状态：HTTP ${result.status}${result.statusText ? ` ${result.statusText}` : ''}`] : []),
        `详情：${result.detail || '无'}`,
        ...(result.note ? [`判断：${result.note}`] : []),
      ]

      setConfirmDialog({
        title: kind === 'connection' ? '接口连接测试结果' : '生图接口测试结果',
        message: lines.join('\n'),
        showCancel: false,
        confirmText: '关闭',
        icon: 'info',
      ...
      showToast(result.ok ? '接口测试成功' : '接口测试失败', result.ok ? 'success' : 'error')
    } finally {
      setRunningDiagnostic(null)
    }
  }, [activeProfile, draft, setConfirmDialog, showToast, timeoutInput])
  ...
  const commitSettings = (nextDraft: AppSettings) => {
    const personalDraft = normalizePersonalDraft(nextDraft)
    const normalizedProfiles = personalDraft.profiles.map((profile) => {
    ...
    const normalizedDraft = normalizeSettings({
      ...
      activeProfileId: normalizedProfiles.some((profile) => profile.id === personalDraft.activeProfileId)
        ? personalDraft.activeProfileId
          ...
          <div className="flex-1 flex flex-col min-w-0 min-h-0 bg-transparent relative overflow-hidden">
                ...
                <div className="rounded-2xl border border-gray-100 bg-white/60 p-3 dark:border-white/[0.06] dark:bg-white/[0.02]">
                  ...
                  >
                    <span>
                      ...
                      <span data-selectable-text className="mt-0.5 block text-xs text-gray-500 dark:text-gray-500">
                        低频的任务卡片显示行为，保持默认即可。
                      </span>
                    </span>
              ...
              <div className="space-y-4">
                ...
                <div className="block">
                  <div className="mb-1 flex items-center justify-between gap-3">
                    ...
                    <div data-selectable-text className="text-xs text-gray-500 dark:text-gray-500">
                      为连续创作启用 Responses API 的 <code className="rounded bg-gray-100 px-1 py-0.5 font-mono text-[10px] dark:bg-white/[0.06]">web_search</code> 工具。模型每次调用此工具会产生少量固定价格的额外计费。
                    </div>
                </div>
                <div className="rounded-2xl border border-gray-100 bg-white/60 p-3 dark:border-white/[0.06] dark:bg-white/[0.02]">
                  <button
                    type="button"
                    onClick={() => setShowAdvancedAgentSettings((value) => !value)}
                    className="flex w-full items-center justify-between gap-3 text-left"
                    aria-expanded={showAdvancedAgentSettings}
                  >
                    <span>
                      <span className="block text-sm font-medium text-gray-700 dark:text-gray-200">高级连续创作限制</span>
                      <span data-selectable-text className="mt-0.5 block text-xs text-gray-500 dark:text-gray-500">
                        防止模型连续调用工具过多；一般不需要调整。
                      </span>
                    </span>
                    <ChevronDownIcon className={`h-4 w-4 shrink-0 text-gray-400 transition-transform ${showAdvancedAgentSettings ? 'rotate-180' : ''}`} />
                  </button>
                  {showAdvancedAgentSettings && (
                    <label className="mt-3 block border-t border-gray-100 pt-3 dark:border-white/[0.06]">
                      ...
                      <div data-selectable-text className="mt-1.5 text-xs leading-relaxed text-gray-500 dark:text-gray-500">
                        默认 15。用于限制连续创作在单次请求中连续调用工具的最大轮数，防止无限循环。
                      </div>
                    </label>
                  )}
                </div>
            ...
            {activeTab === 'api' && (
                ...
                <label className="block">
                  ...
                  <div data-selectable-text className="mt-1.5 min-h-[22px] flex items-center text-xs text-gray-500 dark:text-gray-500">
                    {apiProxyEnabled ? (
                      ...
                      <span>当前实例使用本地保存配置，不再从 URL 查询参数导入接口地址。</span>
                    )}
                  </div>
                </label>
              )}
              ...
              <div className="rounded-2xl border border-gray-100 bg-white/60 p-3 dark:border-white/[0.06] dark:bg-white/[0.02]">
                ...
                >
                  <span>
                    <span className="block text-sm font-medium text-gray-700 dark:text-gray-200">高级连接设置</span>
                    <span data-selectable-text className="mt-0.5 block text-xs text-gray-500 dark:text-gray-500">
                      代理、流式、返回格式、超时和接口诊断。日常只填 Key、URL 和模型即可。
                    </span>
                  </span>
                ...
                {showAdvancedApiSettings && (
                  <div className="mt-3 space-y-4 border-t border-gray-100 pt-4 dark:border-white/[0.06]">
              {/* 4. API 代理 */}
              ...
              {activeProviderIsOpenAICompatible && (
                <div className="rounded-2xl border border-gray-100 bg-white/75 p-4 dark:border-white/[0.06] dark:bg-white/[0.02] space-y-3">
                  <div>
                    <h4 className="text-sm font-bold text-gray-800 dark:text-gray-100">接口诊断</h4>
                    <p className="mt-1 text-xs leading-relaxed text-gray-500 dark:text-gray-500">
                      用当前配置直接测试接口是否可达。连接测试优先检查地址与鉴权；生图接口测试会真实调用一次图像接口，可能消耗额度。
                    </p>
                  </div>
                  <div className="flex flex-col gap-2 sm:flex-row">
                    <button
                      type="button"
                      onClick={() => void runApiDiagnostic('connection')}
                      disabled={runningDiagnostic !== null}
                      className="flex-1 rounded-xl bg-gray-100/90 px-4 py-2.5 text-sm font-medium text-gray-700 transition hover:bg-gray-200 hover:text-gray-900 disabled:cursor-not-allowed disabled:opacity-60 dark:bg-white/[0.06] dark:text-gray-300 dark:hover:bg-white/[0.1] dark:hover:text-white"
                    >
                      {runningDiagnostic === 'connection' ? '测试中...' : '测试连接'}
                    </button>
                    <button
                      type="button"
                      onClick={() => void runApiDiagnostic('image')}
                      disabled={runningDiagnostic !== null}
                    ...
                    >
                      {runningDiagnostic === 'image' ? '测试中...' : '测试生图接口'}
                    </button>
                  </div>
                </div>
              )}
            ...
            {activeTab === 'about' && (
              <div className="flex h-full min-h-[300px] flex-col items-center justify-center pb-8 px-6">
                ...
                <p className="mt-8 mb-6 max-w-[360px] text-center text-[13px] leading-relaxed text-gray-500 dark:text-gray-400">
                  当前版本以个人独立使用为优先，聚焦快速生成、方案迭代、历史复用与后续扩展能力预留。
                </p>
