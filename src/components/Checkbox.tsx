...
export function Checkbox({ checked, onChange, label, tone = 'primary', className, ...props }: CheckboxProps) {
  const toneClasses = tone === 'danger'
    ...
    : 'border-slate-300 bg-white checked:bg-blue-500 checked:border-blue-500 focus:ring-blue-500/20 dark:border-white/25 dark:bg-white/10'
    ...
    <label className={`flex items-center gap-2.5 cursor-pointer group ${className || ''}`}>
      <div className="relative flex items-center justify-center">
        <input
          ...
          className={`peer h-5 w-5 cursor-pointer appearance-none rounded-md border-2 shadow-sm transition-all focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-offset-white dark:focus:ring-offset-gray-900 ${toneClasses}`}
          {...props}
        />
