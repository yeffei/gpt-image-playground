...
export default function Select({ value, onChange, onReorder, options, disabled, ariaLabel, className }: SelectProps) {
  ...
  useEffect(() => {
    ...
    const updateMenuMaxHeight = () => {
      ...
      const left = Math.min(
        Math.max(minViewportInset, rect.left),
        Math.max(minViewportInset, window.innerWidth - menuWidth - minViewportInset),
      )
      const availableBelow = window.innerHeight - rect.bottom - gap - minViewportInset
      const availableAbove = rect.top - gap - minViewportInset
      ...
      const estimatedMenuHeight = Math.min(safeMaxHeight, options.length * 38 + 12)
      const top = newPlacement === 'top'
        ? Math.max(minViewportInset, rect.top - gap - estimatedMenuHeight)
        : Math.min(rect.bottom + gap, window.innerHeight - minViewportInset)
        ...
        >
          {options.map((option) => (
            <div
              ...
              className={`relative flex cursor-pointer items-center justify-between gap-2 px-3 py-2 text-xs transition-colors studio-select-option ${
