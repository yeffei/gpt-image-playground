import type { BackendRoute, TaskParams } from '../types'
...
export async function callGeminiNativeImageApi(input: {
  route: BackendRoute
  ...
  const response = await fetch(buildGeminiModelPath(input.route.baseUrl, input.upstreamModel), {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'x-goog-api-key': input.route.apiKey,
    },
  ...
  return parseGeminiNativeImagePayload(await response.json())
}
