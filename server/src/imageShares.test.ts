...
function createTestDb() {
  ...
  const db = {
    async query(text: string, values?: unknown[]) {
      ...
      if (text.includes('FROM generation_task_outputs') && text.includes('WHERE id = $1 AND user_id = $2')) {
      ...
      if (text.includes('INSERT INTO generation_output_shares')) {
        const [id, token, outputId, userId, accessCodeHash, accessCodeSalt, expiresAt, createdAt] = values ?? []
        const share = {
          id,
          token,
          output_id: outputId,
      ...
      if (text.includes('FROM generation_output_shares') && text.includes('WHERE output_id = $1 AND user_id = $2')) {
        const [outputId, userId] = values ?? []
        return {
          rows: state.shares
            .filter((item) => item.output_id === outputId && item.user_id === userId)
            .slice()
            .reverse(),
        }
      }
      ...
      if (text.includes('UPDATE generation_output_shares')) {
        ...
        const share = state.shares.find((item) => item.id === shareId && item.user_id === userId)
        if (!share) return { rows: [] }
